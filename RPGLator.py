...
# deep_translator'dan sadece GoogleTranslator'ı kullanıyoruz
...
class TranslatorGUI:
        ...
        try:
            ...
            for i in range(0, len(texts_to_translate), self.BATCH_SIZE):
                ...
                try:
                    ...
                    self.log(f"  WARNING: Batch translation failed ({e}). Returning original texts for this batch.", is_error=True)
    ...
    def is_translatable(self, key, value, parent_obj=None):
        ...
        # Dosya yolu / URL
        if re.search(r'\.(ogg|png|m4a|wav|jpg|jpeg|mp3|webp|gif|webm|json|js)$', value, re.IGNORECASE) or re.search(r'[/\\]|^https?://', value):
            return False
